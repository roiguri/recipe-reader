...
const TOKEN_REFRESH_BUFFER_SECONDS = 5 * 60; // 5 minutes in seconds
const MIN_REFRESH_DELAY_MS = 1000; // 1 second minimum delay
const SESSION_CHECK_INTERVAL_MS = 30000; // 30 seconds
const SESSION_EXPIRING_THRESHOLD_SECONDS = 300; // 5 minutes in seconds
const SESSION_CRITICAL_THRESHOLD_SECONDS = 60; // 1 minute in seconds
...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    sessionCheckRef.current = setInterval(() => {
      ...
      if (timeRemaining < SESSION_CRITICAL_THRESHOLD_SECONDS) { // Less than 1 minute remaining
        validateSession();
      }
  ...
  }, [state.session, state.sessionStatus, validateSession]);
  ...
  useEffect(() => {
    ...
    return clearTimers;
  }, [state.session, state.sessionStatus, scheduleTokenRefresh]);
  ...
  const value = {
    ...
    isSessionExpiring: state.session ? getSessionTimeRemaining(state.session) < SESSION_EXPIRING_THRESHOLD_SECONDS : false, // Less than 5 minutes
