import React from 'react';
import { motion } from 'framer-motion';
import { useTranslation } from 'react-i18next';
import { format, parseISO } from 'date-fns';
import Button from '../ui/Button';

/**
 * HistoryRecipeCard component for displaying recipe history items
 */
const HistoryRecipeCard = ({ recipe, onSaveToCollection, onRecipeClick, getStatusIndicator }) => {
  const { t } = useTranslation();
  const statusInfo = getStatusIndicator(recipe.recipe_status);
  
  return (
    <motion.div
      layoutId={recipe.id}
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      className="bg-[#fcf8f8] border border-[#e7d0d1] rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer"
      onClick={() => onRecipeClick(recipe.id)}
    >
      <div className="flex items-center justify-between">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-2">
            <div className="text-[#1b0e0e] flex-shrink-0">
              {recipe.source_type === 'url' ? (
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M16.36,14C16.44,13.34 16.5,12.68 16.5,12C16.5,11.32 16.44,10.66 16.36,10H19.74C19.9,10.64 20,11.31 20,12C20,12.69 19.9,13.36 19.74,14M14.59,19.56C15.19,18.45 15.65,17.25 15.97,16H18.92C17.96,17.65 16.43,18.93 14.59,19.56M14.34,14H9.66C9.56,13.34 9.5,12.68 9.5,12C9.5,11.32 9.56,10.65 9.66,10H14.34C14.43,10.65 14.5,11.32 14.5,12C14.5,12.68 14.43,13.34 14.34,14M12,19.96C11.17,18.76 10.5,17.43 10.09,16H13.91C13.5,17.43 12.83,18.76 12,19.96M8,8H5.08C6.03,6.34 7.57,5.06 9.4,4.44C8.8,5.55 8.35,6.75 8,8M5.08,16H8C8.35,17.25 8.8,18.45 9.4,19.56C7.57,18.93 6.03,17.65 5.08,16M4.26,14C4.1,13.36 4,12.69 4,12C4,11.31 4.1,10.64 4.26,10H7.64C7.56,10.66 7.5,11.32 7.5,12C7.5,12.68 7.56,13.34 7.64,14M12,4.03C12.83,5.23 13.5,6.57 13.91,8H10.09C10.5,6.57 11.17,5.23 12,4.03M18.92,8H15.97C15.65,6.75 15.19,5.55 14.59,4.44C16.43,5.07 17.96,6.34 18.92,8M12,2C6.47,2 2,6.5 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2Z"/>
                </svg>
              ) : recipe.source_type === 'image' ? (
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M8.5,13.5L11,16.5L14.5,12L19,18H5M21,19V5C21,3.89 20.1,3 19,3H5A2,2 0 0,0 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19Z"/>
                </svg>
              ) : (
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M14,2H6A2,2 0 0,0 4,4V20A2,2 0 0,0 6,22H18A2,2 0 0,0 20,20V8L14,2M18,20H6V4H13V9H18V20Z"/>
                </svg>
              )}
            </div>
            <h4 className="text-[#1b0e0e] font-medium truncate">
              {recipe.title || recipe.processed_recipe?.name || t('myRecipes.untitled')}
            </h4>
          </div>
          
          <div className="flex items-center gap-4 text-sm">
            <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-full ${statusInfo.bg} ${statusInfo.color}`}>
              <span>{statusInfo.icon}</span>
              <span>{statusInfo.text}</span>
            </div>
            <span className="text-[#4b2c2c]">
              {format(parseISO(recipe.processed_at || recipe.created_at), 'MMM d, h:mm a')}
            </span>
          </div>
        </div>
        
        {recipe.recipe_status !== 'saved' && recipe.recipe_status !== 'processing' && (
          <Button
            variant="secondary"
            size="sm"
            onClick={(e) => {
              e.stopPropagation();
              onSaveToCollection(recipe.id);
            }}
            className="ml-4"
          >
            {t('myRecipes.history.saveToCollection')}
          </Button>
        )}
      </div>
    </motion.div>
  );
};

export default HistoryRecipeCard;